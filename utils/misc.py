...
def _get_model_analysis_input_few_shot(cfg, use_train_input):
    """
    Return a dummy input for model analysis with batch size 1 and 5-way 1-shot setting.
      The input is used for analyzing the model (counting flops and activations etc.).
    Args:
        cfg (Config): the global config object.
        use_train_input (bool): if True, return the input for training. Otherwise,
            return the input for testing.
    ...
    rgb_dimension = 3
    if use_train_input:
        support_set = torch.rand(cfg.DATA.NUM_INPUT_FRAMES * cfg.TRAIN.WAY * cfg.TRAIN.SHOT, rgb_dimension, cfg.DATA.TRAIN_CROP_SIZE, cfg.DATA.TRAIN_CROP_SIZE)
        target_set = torch.rand(cfg.DATA.NUM_INPUT_FRAMES * cfg.TRAIN.WAY * cfg.TRAIN.QUERY_PER_CLASS, rgb_dimension, cfg.DATA.TRAIN_CROP_SIZE, cfg.DATA.TRAIN_CROP_SIZE)
    else:
        support_set = torch.rand(cfg.DATA.NUM_INPUT_FRAMES * cfg.TRAIN.WAY * cfg.TRAIN.SHOT, rgb_dimension, cfg.DATA.TEST_CROP_SIZE, cfg.DATA.TEST_CROP_SIZE)
        target_set = torch.rand(cfg.DATA.NUM_INPUT_FRAMES * cfg.TRAIN.WAY * cfg.TRAIN.QUERY_PER_CLASS, rgb_dimension, cfg.DATA.TEST_CROP_SIZE, cfg.DATA.TEST_CROP_SIZE)
    support_labels = torch.arange(0, cfg.TRAIN.WAY, dtype=torch.float32)
    if cfg.NUM_GPUS:
        support_set = support_set.cuda(non_blocking=True)
        target_set = target_set.cuda(non_blocking=True)
        support_labels = support_labels.cuda(non_blocking=True)
    inputs = {"support_set": support_set, "target_set": target_set, "support_labels": support_labels}
    return inputs, cfg.TRAIN.WAY * (cfg.TRAIN.SHOT + cfg.TRAIN.QUERY_PER_CLASS)
...
def get_model_stats(model, cfg, mode, use_train_input):
    ...
    try:
        ...
        model.eval()
        with torch.no_grad():
            inputs, num_samples_per_episode = _get_model_analysis_input_few_shot(cfg, use_train_input)
            count_dict, _ = model_stats_fun(model, inputs, _DEFAULT_SUPPORTED_OPS)
            # We log the Flops/Activations per sample
            count = sum(count_dict.values()) / num_samples_per_episode
            if mode == "flop":
                logger.info("Flops: {:,} G".format(count))
            else:
                logger.info("Activations: {:,} M".format(count))
...
def log_model_info(model, cfg, use_train_input=True):
    """
    Log info, includes gflops and activation count.
    ...
    """
    modes=["flop", "activation"]
    for mode in modes:
        get_model_stats(model, cfg, mode, use_train_input)
